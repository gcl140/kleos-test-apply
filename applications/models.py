...
class Application(models.Model):
    ...
    #     if not self.intro_completed and self.agreed:
    #         self.intro_completed = True
    #     if not self.general_info_completed and self.first_name and self.last_name and self.dob and self.gender and self.nationality and self.place_of_birth:
    #         self.general_info_completed = True
    #     if not self.contact_info_completed and self.residency and self.tel1 and self.email1:
    #         self.contact_info_completed = True
    #     if not self.school_info_completed and self.adv_school_name and self.adv_school_phone and self.adv_school_email and self.adv_curriculum and self.olv_school_name and self.olv_school_phone and self.olv_school_email and self.olv_curriculum:
    #         self.school_info_completed = True
    #     if not self.family_info_completed and self.par1_fname and self.par1_lname and self.par1_phone and self.par1_email:
    #         self.family_info_completed = True
    #     # Other sections are completed when at least one item exists
    #     if not self.siblings_completed and self.application_siblings.exists():
    #         self.siblings_completed = True
    #     if not self.dependents_completed and self.application_dependents.exists():
    #         self.dependents_completed = True
    #     if not self.activities_completed and self.application_activities.exists():
    #         self.activities_completed = True
    #     if not self.distinctions_completed and self.application_distinctions.exists():
    #         self.distinctions_completed = True
    #     if not self.other_inquiries_completed and self.form_4_holiday:
    #         self.other_inquiries_completed = True
    #     if not self.writing_completed and self.essay:
